...
def prepare_dataset_for_eval(dataset_name, output_file=None):
    if dataset_name == 'cwq':
...
def align(dataset_name, question_string, question, ground_truth_datas):
    answer_list= []
    
    #print(question_string)
    #print(data)
    #print(ground_truth_datas[0]['question'])
    origin_data = [j for j in ground_truth_datas if j[question_string] == question][0]
    #print(origin_data[0].keys())
    
    if dataset_name == 'cwq':

        if 'answers' in origin_data[0]:
            answers = [one["answers"] for one in origin_data]
        else:
            answers = [one["answer"] for one in origin_data]
...
def save_result2json(dataset_name, num_right, num_error, total_nums, method='none'):
    results_data = {
